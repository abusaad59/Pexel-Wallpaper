...
class WallpaperAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.row_trending_wallpaper, parent, false)
        return ViewHolder(view)
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val model = model[position]
        Glide.with(context).load(model.src.portrait).into(holder.ivImage)
        holder.itemView.setOnClickListener {
            context.startActivity(Intent(context, WallpaperViewActivity::class.java).putExtra("wallpaperUrl",model.src.large).putExtra("photographer",model.photographer))
        }
    }
